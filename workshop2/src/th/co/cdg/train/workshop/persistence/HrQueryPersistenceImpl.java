...
public class HrQueryPersistenceImpl implements HrQueryPersistenceLocal {
	...
	public List<Employee> queryEmployeeByCondition(EmployeeBean employeeBean) {
		if (employeeBean == null || ((employeeBean.getDepartmentBean().getDepartmentCode() == null
				|| "".equals(employeeBean.getDepartmentBean().getDepartmentCode()))
				&& (employeeBean.getJobTitleBean().getJobTitleCode() == null
						|| "".equals(employeeBean.getJobTitleBean().getJobTitleCode()))
				&& (employeeBean.getJobTitleBean().getJobType() == null
		...
		// + " where 1=1");
		//
		// if(employeeBean.getDepartmentBean().getDepartmentCode() != null &&
		// !"".equals(employeeBean.getDepartmentBean().getDepartmentCode())){
		// jpql.append(" and e.departmentCode = :departmentCode");
		// }
		// if(employeeBean.getJobTitleBean().getJobTitleCode() != null &&
		// !"".equals(employeeBean.getJobTitleBean().getJobTitleCode())){
		// jpql.append(" and e.jobTitleCode = :jobTitleCode");
		// }
		// if(employeeBean.getJobTitleBean().getJobType() != null &&
		// !"".equals(employeeBean.getJobTitleBean().getJobType())){
		// jpql.append(" and j.jobType = :jobType");
		// }
		// if(employeeBean.getFirstName() != null &&
		// !"".equals(employeeBean.getFirstName())){
		// jpql.append(" and e.firstName like :firstName");
		// }
		// if(employeeBean.getLastName() != null &&
		// !"".equals(employeeBean.getLastName())){
		// jpql.append(" and e.lastName like :lastName");
		// }
		// if(employeeBean.getGender() != null &&
		// !"".equals(employeeBean.getGender())){
		...
		// employeeBean.getDepartmentBean().getDepartmentCode());
		// }
		// if(employeeBean.getJobTitleBean().getJobTitleCode() != null &&
		// !"".equals(employeeBean.getJobTitleBean().getJobTitleCode())){
		// query.setParameter("jobTitleCode",
		// employeeBean.getJobTitleBean().getJobTitleCode());
		// }
		// if(employeeBean.getJobTitleBean().getJobType() != null &&
		// !"".equals(employeeBean.getJobTitleBean().getJobType())){
		// query.setParameter("jobType",
		// employeeBean.getJobTitleBean().getJobType());
		// }
		// if(employeeBean.getFirstName() != null &&
		// !"".equals(employeeBean.getFirstName())){
		...
		sql.append(
				"select e.employee_id, e.title, e.first_name, e.last_name, e.gender, e.department_code, e.job_title_code, d.department_name, j.job_title_name, j.job_type"
	...
	public Employee queryEmployeeById(String employeeId) {
		...
		final String sql = "select e.employee_id, e.title, e.first_name, e.last_name, e.gender"
				+ ", e.department_code, e.job_title_code, e.address" + ", d.department_name, j.job_title_name"
				+ " from employee e" + " join department d on (e.department_code = d.department_code)"
				+ " join job_title j on (e.job_title_code = j.job_title_code)" + " where e.employee_id = :employeeId"
		...
		if (o != null) {
			...
			JobTitle jTemp = new JobTitle();

			dTemp.setDepartmentCode((String) resultObject[5]);
			jTemp.setJobTitleCode((String) resultObject[6]);
			e.setAddress((String) resultObject[7]);
			dTemp.setDepartmentName((String) resultObject[8]);
			jTemp.setJobTitleName((String) resultObject[9]);
			
			e.setDepartment(dTemp);
			e.setJobTitle(jTemp);
	...
	public List<Skill> querySkillByEmployeeId(String employeeId) {
		...
		final String sql = "select skill_id, employee_id, skill_name, skill_desc" + " from skill"
				+ " where employee_id = :employeeId" + " order by skill_id";
		Query query = em.createNativeQuery(sql);
		...
		if (o != null) {
			...
			if (o instanceof Object[]) {
				...
				Skill s = new Skill();
				s.setSkillId((String) resultObject[0]);

				Employee eTemp = new Employee();
				eTemp.setEmployeeId((String) resultObject[1]);
				s.setEmployee(eTemp);

				s.setSkillName((String) resultObject[2]);
				s.setSkillDesc((String) resultObject[3]);
	...
	public String queryNextEmployeeId() {
		final String sql = "select coalesce(max(employee_id) + 1 , 1) from employee";
		Query query = em.createNativeQuery(sql);
		...
		if (o != null) {
			...
			Integer newEmployeeId = (Integer)resultObject[0];
			
			return String.format("%05d", newEmployeeId);
		}
